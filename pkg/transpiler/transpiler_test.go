...
func normalizeJS(src string) (string, error) {
	cmd := exec.Command("bun", "x", "prettier", "--parser", "babel")
...
func runTranspilerTest(t *testing.T, name, src, expected string) {
	...
	if got != expected {
		...
		normExpected, err1 := normalizeJS(expected)
		normGot, err2 := normalizeJS(got)

		if err1 == nil && err2 == nil && normExpected == normGot {
			return
		}
...
`,
    ...
    let x = 10
    if (x>5) {
        runtime.fmt.println("x is greater than 5")
    }
 else  {
        runtime.fmt.println("x is not greater than 5")
    }

    if (x>20) {
        runtime.fmt.println("x is greater than 20")
    }
 else if (x>15) {
        runtime.fmt.println("x is greater than 15")
    }
 else  {
        runtime.fmt.println("x is small")
    }
